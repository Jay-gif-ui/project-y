...
import { DEFAULT_COUNTRY_CODE, ENABLED_COUNTRIES } from "@/lib/countries";
...
export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    const result = signup
      ? await supabase.auth.signUp({ email: normalizedEmail, password, options: { emailRedirectTo: callbackUrl, data: { ...(name.trim() ? { full_name: name.trim() } : {}), country_code: countryCode } } })
