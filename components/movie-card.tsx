...
export function MovieCard({ movie, index, showRank = false }: { movie: Media; index: number; showRank?: boolean }) {
  ...
  const signal = event ? `${event.status === "upcoming" ? "Coming" : event.kind === "episode" ? "Episode" : event.kind === "digital" ? "Digital" : event.kind === "theatrical" ? "In cinemas" : "Released"} ${date}` : movie.discoverySignal ? signalLabels[movie.discoverySignal] : undefined;
