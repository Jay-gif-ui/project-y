...
export function discoveryParams(type: MediaType, region: string, now = new Date(), options: { activity?: boolean; local?: boolean; genreId?: number; period?: DiscoveryPeriod } = {}): Record<string, string> {
  const dates = discoveryDates(now);
  const start = options.period === "year" ? dates.yearStart : dates.recentStart;
...
export function rankDiscovery(candidates: DiscoveryCandidate[], trending: Media[], now = new Date(), options: { limit?: number; balanceLocal?: boolean; period?: DiscoveryPeriod } = {}): Media[] {
  ...
  while (selected.length < limit) {
    ...
    // Two unrestricted picks, then a local-origin pick if TMDB supplies one.
    const localSlot = options.balanceLocal && selected.length % 3 === 2;
    const next = (localSlot ? available.find(item => item.local) : undefined) ?? available[0];
    selected.push(next);
