...
export async function getWatchProviders(type: MediaType, id: number, region: string): Promise<TmdbResult<WatchProviders>> {
  if (!Number.isSafeInteger(id) || id < 1) return { error: "not-found" };
  const normalizedRegion = region.toUpperCase();
  if (!/^[A-Z]{2}$/.test(normalizedRegion)) return { error: "not-found" };
  const result = await request<Record<string, unknown>>(`/${type}/${id}/watch/providers`);
