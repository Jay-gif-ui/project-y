...
export type ReleaseCandidate = { media: Media; events: ReleaseEvent[]; available?: boolean };
...
export function selectReleaseItems(candidates: ReleaseCandidate[], now = new Date(), status: ReleaseFilter = "all"): Media[] {
  ...
  for (const { media, events, available } of candidates) {
    ...
    if (!event) continue;
    const item: Media = { ...media, releaseAvailability: available === undefined ? "unknown" : available ? "offers-reported" : "not-found", discoverySignal: event.date > today ? "upcoming-release" : "recent-release", releaseEvent: { ...event, status: event.date > today ? "upcoming" : "released" } };
