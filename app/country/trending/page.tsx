...
export default async function CountryTrending({ searchParams }: { searchParams: Promise<{ type?: string | string[]; sort?: string | string[]; period?: string | string[]; page?: string | string[]; origin?: string | string[] }> }) {
  ...
  return <><Navbar /><CountryTrendingPage initialRegion={country.code} filter={filter} origin={origin} sort={sort} period={period} page={result.data?.page ?? 1} total={result.data?.total ?? 0} localTotal={result.data?.localTotal ?? 0} internationalTotal={result.data?.internationalTotal ?? 0} totalPages={result.data?.totalPages ?? 1} movies={result.data?.items ?? []} partial={result.partial ?? false} error={Boolean(result.error)} year={discoveryDates(now).year} /><Footer /></>;
}
