...
export const metadata = { title: "Latest releases in your country | iFynex", description: "Recent and upcoming movies, series and episodes relevant to your country." };

export default async function CountryReleases({ searchParams }: { searchParams: Promise<{ status?: string | string[]; type?: string | string[]; period?: string | string[]; page?: string | string[]; origin?: string | string[] }> }) {
  const country = getCountry((await cookies()).get(COUNTRY_PREFERENCE_COOKIE)?.value);
  const { status: requestedStatus, type, period: requestedPeriod, page: requestedPage, origin: requestedOrigin } = await searchParams;
  const status = requestedStatus === "released" || requestedStatus === "upcoming" ? requestedStatus : "all";
