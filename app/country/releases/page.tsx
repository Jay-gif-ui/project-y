...
export default async function CountryReleases({ searchParams }: { searchParams: Promise<{ type?: string | string[]; period?: string | string[]; page?: string | string[]; origin?: string | string[] }> }) {
  ...
  return <><Navbar /><CountryTrendingPage view="releases" initialRegion={country.code} filter={filter} origin={origin} sort="newest" period={period} page={result.data?.page ?? 1} total={result.data?.total ?? 0} totalPages={result.data?.totalPages ?? 1} movies={result.data?.items ?? []} partial={result.partial ?? false} error={Boolean(result.error)} year={now.getUTCFullYear()} /><Footer /></>;
}
