...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!code) { setMessage("This confirmation link is invalid or has expired. Please request a new one."); return; }
    supabase.auth.exchangeCodeForSession(code).then(({ error }) => { if (error) { setMessage("This confirmation link is invalid or has expired. Please request a new one."); return; } router.replace(destination); router.refresh(); });
  }, [router, searchParams]);
